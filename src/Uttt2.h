...
struct Uttt2: public wag::Game {
    ...
    friend std::ostream& operator<< ( std::ostream& os, Uttt2& c )
    {
        for ( int i = 0; i < 21; i++ ) {
            os << "=";
        }
...
unordered_map<int, unordered_map<int, Uttt2::Move*>> Uttt2::everyMove = unordered_map<int, unordered_map<int, Uttt2::Move*>>();
